...
def get_distance(origins, destinations):
    ...
    API = f"https://maps.googleapis.com/maps/api/distancematrix/json?origins={origins}&destinations={destinations}&key={API_KEY}"
...
def optimise(params: OptimisationQuery):
    ...
    distance_data = get_distance([(car.lat, car.lon) for car in params.cars], [(emergency.lat, emergency.lon) for emergency in params.emergencies])
    ...
    if result is not None:
        ...
        for assignment in result['assignments']:
            ...
            destinations.append(Destination(car_id=car_id, emergency_id=emergency_id, emergency_lat=emergency_lat, emergency_lon=emergency_lon, time_seconds=D[car_index][emergency_index]))
